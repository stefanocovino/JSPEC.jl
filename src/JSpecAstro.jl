...
Create a JSpecAstro data set entry.
...
function CreateDataSet(Name::String, Instrument::String; verbose=true)::Dict
    nd = Dict()
    if uppercase(Instrument) in [uppercase(i) for i in JSpecAstro.KnownInstruments]
        nd["Name"]=Name
...
- `datasets` array of JSpecAstro dictionaries.
...
- `ds` JSpecAstro data set dictionary.
...
function GenResponseMatrix(ds::Dict; verbose=true)
    if "RebinnedData" in keys(ds) && ds["RebinnedData"] && "RebinnedAncillaryData" in keys(ds) && ds["RebinnedAncillaryData"]
        ...
        for i in 1:size(vt)[2]
            vt[:,i] = JSpecAstro.GenRebin(matx[i],ds["RebinSchema"])
        end
...
Returns the instruments currently supported by the JSpecAstro package.
...
function ImportData(ds::Dict; rmffile::String="", arffile::String="", srcfile::String="", bckfile::String="", verbose=true)
    ...
    elseif uppercase(ds["Instrument"]) ∉ [uppercase(i) for i in JSpecAstro.KnownInstruments]
        if verbose
...
- `dts` array of JSpecAstro dictionaries.
...
JSpecAstroFunc([N,λ], [Optdt,XRTdt], Myfunc)
```
"""
function JSpecAstroFunc(pars,dts,inpfnc)
...
- `ds` JSpecAstro data set disctionary.
- `verbose` enables warning messages.
...
function RebinAncillaryData(ds::Dict; verbose=true)
    if "RebinnedData" in keys(ds) && !ds["RebinnedData"]
        ...
        ds["RebinnedMaskedEnergy"] = JSpecAstro.GenRebin(ds["MaskedChannels"][!,"E"],ds["RebinSchema"])
        ds["RebinnedMaskedChannel"] = JSpecAstro.GenRebin(ds["MaskedChannels"][!,"CHANNEL"],ds["RebinSchema"])
...
function RebinData(ds::Dict;minSN=5,verbose=true)
    if !ds["IgnoredChannels"]
        ...
        ds["RebinSchema"] = JSpecAstro.FindRebinSchema(ds["MaskedInputData"],ds["MaskedInputDataErr"],minSN=minSN)
